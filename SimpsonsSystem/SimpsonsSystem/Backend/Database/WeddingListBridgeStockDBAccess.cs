...
namespace SimpsonsSystem.Backend.Database
{
    public class WeddingListBridgeStockDBAccess:IDisposable
    {
        ...
        //Decrement the AmountHeld value by 1 from specified register.
        public void DecrementAmountFromWeddingListBridgeStock(int w,int s,int amt)
        {
            string SQLCmd="UPDATE WeddingListBridgeStock SET StockAmount="+(amt-1)+" WHERE WeddingListId="+w+" AND StockId ="+s+";";
            Classes.ClsSystem.RunSQLNonSelect(SQLCmd,db);
        }
